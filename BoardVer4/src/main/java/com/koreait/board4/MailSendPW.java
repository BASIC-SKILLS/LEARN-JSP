...
public class MailSendPW {
	public void idMailSend(HisVo hisVo) {
		...
		try {
			...
			String mailSubject = hisVo.getHisName()+"("+hisVo.getHisId()+")"+"("+hisVo.getHisGender()+")"+"님의 비밀번호를 알려드립니다.";
			String mailText = "hisVo.getHisName()"+"("+hisVo.getHisId()+")"+"("+"hisVo.getHisGender()"+")"
					+"님의 ★임시★비밀번호는 ↓\n"
					+hisVo.getHisPw()+"\n↑ 이랍니다~^^";
